...
@section('contenedor')
    <div class=" container rounded shadow p-3">
        <div class="row">
            ...
            <div class="col-md-9">
                ...
                <p><strong>Dirección </strong>{{ Auth::user()->personaUser->refugio->dir_refugio}}</p>
    ...
    <div class="modal fade" id="modal-editar-empresa">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{url ('refugio/editar')}}" method="post" id="formeditrefugio">
                    {!! csrf_field() !!}
                    <div class=" modal-body">
                        <div class=" form-group">
                            <label for="">Nombre:</label>
                            <input type="text" id="txtnomrefugio" name="txtnomrefugio" class=" form-control form-control-sm" value="{{ Auth::user()->personaUser->refugio->nom_refugio}}">
                            <div class="invalid-feedback" id="error-nomrefugio"></div>
                        </div>
                        <div class="row">
                            <div class="form-group col-6">
                        ...
                        <div class=" form-group">
                            <label for="">Dirección:</label>
                            <input type="text" id="txtdireccionref" name="txtdireccionref" class=" form-control form-control-sm" value="{{ Auth::user()->personaUser->refugio->dir_refugio}}">
                            <div class="invalid-feedback" id="error-direccionref"></div>
                        </div>
                        <div class=" form-group">
                            <label for="">Descripción:</label>
                            <input type="text" id="txtdescripcion" name="txtdescripcion" class=" form-control form-control-sm" value="{{ Auth::user()->personaUser->refugio->desc_refugio}}">
                            <div class="invalid-feedback" id="error-descripcion"></div>
                        </div>
                        <div class="row">
                            <div class=" form-group col-6">
                                <label for="">capacidad:</label>
                                <input type="number" id="txtcapacidad" name="txtcapacidad" class=" form-control form-control-sm" value="{{ Auth::user()->personaUser->refugio->capacidad}}">
                                <div class="invalid-feedback" id="error-capacidad"></div>
                            </div>
                            <div class="form-group col-6">
                                <label for="">Tipo de Refugio</label>
                                <select class="custom-select custom-select-sm" id="txttipo" name="txttipo" required>
                                    <option value="{{Auth::user()->personaUser->refugio->pktipo}}">{{ Auth::user()->personaUser->refugio->refugiotipo->tipo_refugio}}</option>
                                    @foreach ($tipos as $tipo)
                                        @if ($tipo->id != Auth::user()->personaUser->refugio->pktipo)
                    ...
                    <div class=" modal-footer">
                        <button type="button" class="btn btn-outline-secondary" data-dismiss="modal">Cancelar</button>
                        <button type="button" class="btn btn-info" onclick="validar_refugio()">Guardar</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        ...
        function validar_refugio(){
            $.ajax({
                url: "{{ route('refugio.validar') }}",
                method: "POST",
                data: $('#formeditrefugio').serialize(),
                success: function (data) {
                    if ((data.nomrefugio).length > 0) {
                        error_view('nomrefugio', data.nomrefugio)
                    }
                    ...
                    if ((data.direccionref).length > 0) {
                        error_view('direccionemp', data.direccionref)
                    }
                    if ((data.descripcion).length > 0) {
                        error_view('descripcion', data.descripcion)
                    }
                    if ((data.capacidad).length > 0) {
                        error_view('capacidad', data.capacidad)
                    }
                    if((data.nomrefugio).length == 0 && (data.email).length == 0 && (data.password).length == 0 && (data.direccionref).length == 0 && (data.descripcion).length == 0 && (data.capacidad).length == 0){
                        $( "#formeditrefugio" ).submit();
                    }
                }
            });
        }
