...
@section('contenedor')
    <div class=" container rounded shadow p-3">
        <div class="row">
            {{-- <div class="col-3 bg-dark">
                <img src="{{ asset('images/imagenblanco.png')}}" class="img-fluid rounded-circle" style="width:160px; height:160px">
            </div>
            <div class="col-9">
                <h5>Perfil</h5>
            </div> --}}
            ...
            <div class="col-md-9">
                <div class="row">
                    <div class="col-8 col-md-10">
                        <h5>
                            {{ Auth::user()->personaUser->nom_persona.' '.Auth::user()->personaUser->apellido }}
                        </h5>
                    </div>
                    <div class="col-4 col-md-2">
                        <button type="button" class="btn btn-sm btn-outline-secondary profile-edit-btn border-0" data-toggle="modal" data-target="#modal-editar" >Editar Perfil</button>
                    </div>
                </div>
    ...
    <div class="modal fade" id="modal-editar">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{url ('activista/editar')}}" method="post" id="formeditaractivista">
                    {!! csrf_field() !!}
                    <div class=" modal-body">
                        <div class=" form-group">
                        ...
                        <div class="row">
                            <div class=" form-group col-7">
                                ...
                                <input type="email" id="txtemail" name="txtemail" class=" form-control form-control-sm" value="{{Auth::user()->email}}">
                    ...
                    <div class=" modal-footer">
                        <button type="button" class="btn btn-outline-secondary" data-dismiss="modal">Cancelar</button>
                        <button type="button" class="btn btn-info" onclick="validar_perfil()">Guardar</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        ...
        function validar_perfil(){
            $.ajax({
                url: "{{ route('activista.validar') }}",
                method: "POST",
                data: $('#formeditaractivista').serialize(),
                success: function (data) {
                    ...
                    if((data.password).length == 0 && (data.date).length == 0 && (data.nombre).length == 0 && (data.apellido).length == 0 && (data.email).length == 0 && (data.direccion).length == 0 && (data.ci).length == 0 && (data.telefono).length == 0 && (data.genero).length == 0){
                        $( "#formeditaractivista" ).submit();
                    }
                }
            });
        }
